...
import os
import torch
from .utils import merge_images, load_images, chat_mllava
...
class MLlava():
    ...
    def __init__(self, model_path:str="Mantis-VL/mllava_nlvr2_4096") -> None:
        ...
        if "llava_next" in model_path:
            ...
            use_adape = True if 'ada' in model_path else False
            use_adape = False
            self.model = LlavaForConditionalGeneration.from_pretrained(model_path, use_adape, device_map="cuda", torch_dtype=torch.bfloat16, attn_implementation=attn_implementation).eval()
