...
class PELlamaAttention(nn.Module):
    ...
    def forward(
        ...
        position_states = tuple(
            shift(i, bsz, q_len, group_size, self.num_heads, self.head_dim) for i in position_states
        )
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.rotary_emb = MyLlamaRotaryEmbedding(config.hidden_size // config.num_attention_heads, config.num_attention_heads, max_position_embeddings=config.max_position_embeddings, scaling_factor=self.config.rope_scaling)
